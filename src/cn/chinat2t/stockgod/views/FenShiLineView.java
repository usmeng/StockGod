...
public class FenShiLineView extends GridChart{
	...
	public FenShiLineView(Context context) {
		super(context);
	}

	public FenShiLineView(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
	}

	public FenShiLineView(Context context, AttributeSet attrs) {
		super(context, attrs);
	}
...
//		drawLineTitle(canvas);
	...
	public void setLineData(List<LineEntity> lineData) {
		...
		if(lineData != null && lineData.size() > 0 && type != 1){
	...
	protected void initMaxAndMin() {
				...
				if (lineData != null) {
					for (int j = 0; j < maxCandleSticksNum && j + startIndex < lineData.size(); j++) {
						...
						if (value > getMaxPrice()) {
							setMaxPrice(value);
						} else if (value > 0 && value < getMinPrice()) {
							setMinPrice(value);
						}
					}
				}
		...
		if(type == 1){
			if(getMaxPrice() - nowclose > nowclose - getMinPrice()){
				setMinPrice(nowclose - (getMaxPrice() - nowclose));
			}else{
				setMaxPrice(nowclose + (nowclose - getMinPrice()));
			}
		}
	}
		...
		if (null != OHLCData) {
			...
			for (int i = 0; i < longtitudeNum; i++) {
				...
				if (index + startIndex < OHLCData.size()) {
					TitleX.add(StringUtil.getFenShiTime(OHLCData.get(index + startIndex).getDate()));
				} else {
					TitleX.add(StringUtil.getFenShiTime(OHLCData.get(OHLCData.size() - 1).getDate()));
				}
			}
			if (startIndex + maxCandleSticksNum < OHLCData.size()) {
				TitleX.add(StringUtil.getFenShiTime(OHLCData.get(startIndex + maxCandleSticksNum).getDate()));
			} else {
				TitleX.add(StringUtil.getFenShiTime(OHLCData.get(OHLCData.size() - 1).getDate()));
			}
		}
	...
	protected void initAxisY() {
		...
		List<String> TitleY2 = new ArrayList<String>();
		...
		for (int i = 0; i < latitudeNum; i++) {
			...
			TitleY2.add(DataUtil.getDouble1((Float.parseFloat(value) - nowclose) * 100 / nowclose) + "%");
		}
		// 最大值
		...
		super.setAxisRightYTitles(TitleY2);
	}
	...
	*/}
	...
	protected void drawWithFingerClick(Canvas canvas) {
		...
		System.out.println("ohlc.navprice---2:" + ohlc.navprice);
		System.out.println("ohlc.noeprice---2:" + ohlc.noeprice);
		if(ohlc.getHigh() <= 0 && ohlc.getClose() <= 0 && ohlc.getOpen() <= 0) return;
		if(showCandleSticks){
	...
	public void drawPopMune(Canvas canvas, float pointX, KlineData ohlc,
		...
		if (type == 1) {
			...
			list.add(new StringView(StringUtil.getFenShiTime(ohlc.getDate()), Color.WHITE));
			list.add(new StringView("价格："));
			System.out.println("ohlc.navprice: " + ohlc.navprice);
			System.out.println("ohlc.noeprice:" + ohlc.noeprice);
